...
pub struct TypeScriptWasiAdapter;

impl Adapter for TypeScriptWasiAdapter {
    ...
    fn adapt(&self, subject: &SubjectManifest) -> Result<AdaptationReport> {
        if subject.capabilities.contains(&Capability::McpProtocol) {
            return Ok(AdaptationReport {
                ...
                notes: vec![],
                blockers: vec![
                    "TypeScriptWasiAdapter does not support mcp-protocol; use NativeMcpAdapter"
                        .to_string(),
                ],
            });
        }
        ...
        if subject.language != Language::TypeScript && subject.language != Language::JavaScript {
            return Ok(AdaptationReport {
                ...
                notes: vec![],
                blockers: vec![
                    "TypeScriptWasiAdapter only supports TypeScript/JavaScript subjects"
                        .to_string(),
                ],
            });
        }
        ...
        Ok(AdaptationReport {
            ...
            notes: vec!["compiled with Javy WASI build command".to_string()],
            blockers: vec![],
        })
    }
}
...
mod tests {
    ...
    fn adapts_ts_env_leak_to_wasm() {
        ...
        let adapter = TypeScriptWasiAdapter;
