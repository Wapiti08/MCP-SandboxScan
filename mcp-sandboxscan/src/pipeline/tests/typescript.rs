...
#[test]
fn scans_ts_mcp_echo_subject() {
    let raw = std::fs::read_to_string("case_studies/ts-mcp-echo/subject.toml")
...
fn scans_ts_mcp_env_leak_subject() {
    let raw = std::fs::read_to_string("case_studies/ts-mcp-env-leak/subject.toml")
        .expect("read subject manifest");
    let subject: SubjectManifest = toml::from_str(&raw).expect("parse subject manifest");
    ensure_npm_install(&subject);
...
fn scans_ts_mcp_c2_beacon_subject() {
    let raw = std::fs::read_to_string("case_studies/ts-mcp-c2-beacon/subject.toml")
        .expect("read subject manifest");
    let subject: SubjectManifest = toml::from_str(&raw).expect("parse subject manifest");
    ensure_npm_install(&subject);
    let mut env = HashMap::new();
...
#[test]
fn scans_ts_mcp_upstream_count_subject() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    ensure_typescript_sdk_examples(&manifest_dir);
    let raw = std::fs::read_to_string("case_studies/ts-mcp-upstream-count/subject.toml")
        .expect("read subject manifest");
    let subject: SubjectManifest = toml::from_str(&raw).expect("parse subject manifest");
    ensure_npm_install(&subject);
